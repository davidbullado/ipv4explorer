...
function getLatLng(map: L.Map, point: IPoint): L.LatLng {
    let ret: L.LatLng;
    try {
        let p: L.Point = new L.Point(point.x, point.y);
...
function poligonizify(iprange: string, label: string, color: string) {

    const twopart: string[] = iprange.split('/');
